...
export async function getTimeline() {
  ...
  const rowsWithMetadata = await Promise.all(result.rows.map(async (row) => {
    const { description, image, url, title } = await urlMetadata(row.post_link, { description: true, image: true, url: true, title: true });
    return { 
      ...row, 
